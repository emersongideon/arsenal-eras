    ...
    >
      <Reveal>
        ...
        <p className="lead narrow">
          Before crowning anyone, three things that shifted between the eras. These aren't
          inputs to the points model - they're the context around it. Some are hard fact,
          one is frankly a guess. Each is labelled.
        </p>
      </Reveal>
      ...
      <Reveal delay={60}>
        <div className="card spec" style={{ margin: "26px 0 18px" }}>
          ...
          <p>
            {era.var.headline} The Premier League introduced VAR in{" "}
            {era.var.var_introduced}, so a chunk of 2025/26's marginal calls - tight
            offsides, handball penalties - simply couldn't have been reviewed 22 years
            earlier.
          </p>
          ...
          <div style={{ fontSize: 15 }}>
            ...
            <b style={{ color: "#b4550a" }}>
              {era.var.estimated_points_swing.low} to +
              {era.var.estimated_points_swing.high} pts
            </b>
            <ul style={{ marginTop: 10 }}>
              {era.var.assumptions.map((a) => (
                <li key={a} className="dim" style={{ fontSize: 14 }}>
                  {a}
                </li>
              ))}
            </ul>
      ...
      <Reveal delay={60}>
        <div className="card" style={{ marginBottom: 18 }}>
          ...
          <p>
            {era.fixture_load.headline} The expanded Champions League is the driver:{" "}
            ...
            <b>{era.fixture_load.total["2003/04"]}</b>.
          </p>
      ...
      <Reveal delay={60}>
        <div className="card">
          ...
          <p>
            Derived from the real results: points-per-game against the division's bottom
            half versus its top rivals. The contrast is striking. The Invincibles were{" "}
            <b>evenly excellent</b> - ~{sd["2003/04"].ppg_vs_top_rivals.toFixed(1)} PPG
            against the best, ~{sd["2003/04"].ppg_vs_bottom_half.toFixed(1)} against the
            rest. The 2025/26 side <b>feasted on weaker teams</b> (
            {sd["2025/26"].ppg_vs_bottom_half.toFixed(1)} PPG, dropping just{" "}
            {sd["2025/26"].points_dropped_vs_bottom_half} points to the bottom half) but{" "}
            <b>struggled against the top</b> ({sd["2025/26"].ppg_vs_top_rivals.toFixed(1)}{" "}
            PPG) - where all five defeats lived.
          </p>
